...
$(document).ready(function() {
	...
	manageProductTable = $('#manageStockTable').DataTable({
	...
	$("#addStockModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitStockForm").unbind('submit').bind('submit', function() {
			...
			if(stockName == "") {
				$("#stockName").after('<p class="text-danger">Stock Name field is required</p>');
				$('#stockName').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#stockName").find('.text-danger').remove();
				// success out for form 
				$("#stockName").closest('.form-group').addClass('has-success');	  	
			}	// /else
			...
			if(threshold == "") {
				$("#threshold").after('<p class="text-danger">Threshold field is required</p>');
				$('#threshold').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#threshold").find('.text-danger').remove();
				// success out for form 
				$("#threshold").closest('.form-group').addClass('has-success');	  	
			}	// /else
           
			if(stockName && quantity && unit && threshold) {
	...
	if(productId) {
		...
		$.ajax({
			...
			// alert(response);
				...
				// product id 
				$(".editStockFooter").append('<input type="hidden" name="productId" id="productId" value="'+response.id+'" />');		
				
				// product name
				$("#editStockName").val(response.stock_name);
				// quantity
				$("#editQuantity").val(response.quantity);
				// rate
				$("#editUnit").val(response.unit);
				// brand name
				$("#editThreshold").val(response.threshold);
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					var productName = $("#editStockName").val();
					...
					if(productName == "") {
						$("#editStockName").after('<p class="text-danger">Product Name field is required</p>');
						$('#editStockName').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editStockName").find('.text-danger').remove();
						// success out for form 
						$("#editStockName").closest('.form-group').addClass('has-success');	  	
					}	// /else
					...
					if(unit == "") {
						$("#editUnit").after('<p class="text-danger">Rate field is required</p>');
					...
					if(threshold == "") {
						$("#editThreshold").after('<p class="text-danger">Brand Name field is required</p>');
						$('#editThreshold').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editThreshold").find('.text-danger').remove();
						// success out for form 
						$("#editThreshold").closest('.form-group').addClass('has-success');	  	
					}	// /else					

					if(productName && quantity && unit && threshold) {
						// submit loading button
